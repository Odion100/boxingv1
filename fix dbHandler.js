...
orders.vaildations = {
    dates:['createdDate'],
    numbers:['multiple', 'item_weight', 'pickedUnits', 'boxNumber', 'sessionCount', 'vCode', 'weekNum', 'total_weight'],
    booleans:[],
    json:[], 
    object_id:[],
    tests:[],
...
function setXlHandler(coll, updateFields, insert_cb, update_cb){    
    ...
    coll.xlFileHandler = new multiTaskHandler(function(file){
        ...
        function insert(nextTask){              
            ...
            //temp
            var new_boxes = [], box_ids = uniqueKeys(new_json, 'box_id')
            
            box_ids.forEach(function(box_id){
                var boxMatches = findByKey(new_json, 'box_id', box_id, true);
                
                var box = cloneKeys(boxMatches[0], ['amazon_po', 'boxNumber', 'box_id', 'created_date', 'fulfillment_center', 'open', 'session', 'sessionCount', 'sessionStatus', 'session_id', 'vCode', 'vendor_code', 'weekNum', 'labelNumber'])
                
                box.boxContents = []
                
                boxMatches.forEach(function(boxData){
                    var item = cloneKeys(boxData, ['asin', 'item_weight', 'multiple', 'pf_sku', 'title', '_id', 'pickedUnits']);
                    item._id = boxData.amazon_po + boxData.asin;
                    box.boxContents.push(item)
                }) 
...
new setXlHandler(boxes, []);


function asinUpdate_cb(){
...
var updateNullEaches = new multiTaskHandler(function(){
    ...
    function getNulls(nextTask){
        ...
        }, {$or:[{pending_eaches:null}, {pending_eaches:NaN}]})
    }
...
var receiptsProcessor = new multiTaskHandler(function(pf_recs){
 ...
 var receiptHandler = new multiTaskHandler(function(){
    ...
    function getUnusedReceipts(nextTask){
        receipts.find({unused_qty:{$gt:0}}, function(err, doc){
             if(err){
                nextTask(err);
            }else{       
                pf_recs = doc;
    ...
    function getLastCheckDate(nextTask){
        ...
        aux.find({_id:"new_skus_check"}, function(err, doc){
            if(err){
                ...
                console.log(doc[0])              
                last_check = (doc[0])?moment(doc[0].last_check).format("YYYY-MM-DD"): moment().format("YYYY-MM-DD");                
                nextTask();
            }
        })
    }
...
var boxingInit = function(){
    ...
    var newBoxProcessor = new multiTaskHandler(function(new_boxes){
        var session, vCode, nextBoxNum, sessionCount, weekNum, processedBoxes = [], emptyBoxes = [];
        ...
        function checkForOpenSession(nextTask){
            ...
            _query.$and = [{sessionStatus:'open'}, {vendor_code:new_boxes[0].vendor_code}, {fulfillment_center:new_boxes[0].fulfillment_center}]
        ...
        function generateNewSession(nextTask){
            ...
            boxes.find(_query).limit(1).sort({sessionCount:-1}, function(err, doc){
                if(err){
                    ...
                    session = alpha_codes[sessionCount] + '' + weekNum;                    
                    vCode = doc[0].vCode;                                        
                    nextTask();
                }else{
                   sessionCount = 1;
        ...
        function getEmptyBoxes(nextTask){
            ...
            _query.$and = [{sessionStatus:'open'}, {vendor_code:new_boxes[0].vendor_code}, {fulfillment_center:new_boxes[0].fulfillment_center}, {boxContents:[]}]
        ...
        function addBoxDetails(nextTask){            
            ...
            for (var i = 0; i < new_boxes.length; i++) {
                ...
                /*new_boxes[i].box_id = vCode + new_boxes[i].fulfillment_center + session + leadingZeros(nextBoxNum, 3);*/
        ...
        return {
            ...
            _return:function(){
                for (var i = 0; i < emptyBoxes.length; i++) {
                    processedBoxes.push(emptyBoxes[i])
                }
        ...
        boxes.findOne({_id:request.box_id}, function(err, doc){
            if(err){
                ...
                for (var i = 0; i < doc.boxContents.length; i++) {
                    if(doc.boxContents[i].asin === request.pickedItem.asin){                       
        ...
        orders.findOne({_id:pickedOrder._id}, function(err, doc){
            if(err){
                ...
                if(doc.in_stock_units === pickedOrder.in_stock_units){                    
                    ...
                    doc.in_stock_units = doc.in_stock_units - pickedUnits;
