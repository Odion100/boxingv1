...
.service('$$msgbox', function(){
    ...
    //executes an multiTaskHandler while msgbox is disabled
    //so if you have along multiTaskHandler with some msgbox there in 
    msgbox.disableWhile = function(mth, callBack, args){        
        
        msgbox.enabled = false;
        var cb = function(err, results){
            console.log('multiTaskHandler')
            msgbox.reset();
            if(err){
                callBack(err);
            }else{
