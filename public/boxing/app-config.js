...
.config([
    ...
    function($urlRouterProvider, $stateProvider){
        $urlRouterProvider.otherwise('/order-picking/NJ/login')

        $stateProvider
        .state('orderSearch', {
            url:'/order-picking/:location/:client_id',
...
.service('stateHandler', ['_client', '$$db', 'tasks', '$state', '$stateParams', '_client', '$$db', function(_client, $$db, tasks, $state, stateParams, _client, db){
    ...
    function updateState(){
        var loc = (stateParams.location.toUpperCase() === 'NY')?'NY':'NJ'; 
        $state.go('orderSearch', {client_id:sh.user._id, location:loc}, {notify:true})        
    }
    function logout(){
